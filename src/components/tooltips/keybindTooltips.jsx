...
export function RowTooltip({children, text, keybind}) {
...
export function ColumnTooltip({children, text, keybind}) {
    const [showTooltip, setShowTooltip] = useState(false);
    ...
    useEffect(() => {
        ...
        if (showTooltip) {
            ...
            const tooltipTop = tooltipRect.top + tooltipHeight; // Calculate the right edge including the width

            if (window.innerWidth < tooltipTop + 10) {
                // Adjusted to add a small buffer
                setTooltipPosition("bottom-0");
            } else {
                setTooltipPosition("top-0");
            }
        }
        // Cleanup
        return () => {
    ...
    return (
        ...
        >
            ...
            {showTooltip && (
                <div
                    ref={tooltipRef}
                    className={`absolute left-full ${tooltipPosition} ml-2 w-max p-2 flex items-center dark:text-white bg-slate-200 dark:bg-slate-950 rounded-md z-10`}
                >
                    <p className="text-sm">{text}</p>
                    {keybind && (
                        <div className="flex items-center ml-1">
